...
import { rejectArticleTransfer, rejectArticleTransferVariables } from '../../../queries/__generated__/rejectArticleTransfer';
...
export const rejectArticleTransferEpic = (
    ...
    action$.pipe(
        ...
        mergeMap(({ payload: { id } }) =>
            apolloClient.mutate<rejectArticleTransfer, rejectArticleTransferVariables>({
        ...
        flatMap(
            ({
                data,
            }) => apolloSubscriber(data && data.rejectArticleTransfer && data.rejectArticleTransfer.hash || '')
        ),
